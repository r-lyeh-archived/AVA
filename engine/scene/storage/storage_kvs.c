...
#if !defined(KVS_KEY_LEN) && !defined(KVS_VAL_LEN)
...
enum {
    KVS_HASH_BUCKET = 508 /*1024*/,
    KVS_KEY_LEN = 31+1,
    KVS_VAL_LEN = 508 - KVS_KEY_LEN
};
#endif

typedef char kvs_key[KVS_KEY_LEN];
typedef char kvs_val[KVS_VAL_LEN];

API bool kvs_read( const char *dbfile, const char *keystr, char *val );
API bool kvs_write( const char *dbfile, const char *keystr, const char *val );
...
bool kvs_read( const char *dbfile, const char *keystr, char *val ) {
    KISSDB kdb = {0};
    int ok = 0 == KISSDB_open( &kdb, dbfile, KISSDB_OPEN_MODE_RWCREAT, KVS_HASH_BUCKET, KVS_KEY_LEN, KVS_VAL_LEN);
    if( ok ) {
        kvs_key key = {0};
        strcpy( key, keystr ); //sprintf(key, "%.*s", strlen(keystr) > KVS_KEY_LEN ? KVS_KEY_LEN : strlen(keystr), keystr);
...
bool kvs_write( const char *dbfile, const char *keystr, const char *val ) {
    KISSDB kdb = {0};
    int ok = 0 == KISSDB_open( &kdb, dbfile, KISSDB_OPEN_MODE_RWCREAT, KVS_HASH_BUCKET, KVS_KEY_LEN, KVS_VAL_LEN);
    if( ok ) {
        kvs_key key = {0};
...
#include <stdio.h>
int main( int argc, char **argv ) {
    kvs_val val = "0"; // default value, if not found
    kvs_read( "db.kvs", "/hello", val );

    printf("/hello=%s\n", val);

    val[0] ++; if( val[0] > '9' ) val[0] = '0';
    kvs_write( "db.kvs", "/hello", val );
}
#endif
