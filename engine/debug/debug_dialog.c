...
int dialog( int buttons, const char *title, const char *message );
...
#include <stdio.h>
int main() {
    int unicode = dialog(0, "unicode test info dialog", "私 は ガ");

    int pressed1 = dialog(3, "3-buttons general dialog", "Bla.\nContinue?");
    int pressed2 = dialog(2, "2-buttons warning dialog", "Bla.\nContinue?");
    int pressed3 = dialog(1, "1-button error dialog", "Bla.\nBla.");
    int pressed4 = dialog(1, "1-button about dialog", "Bla.\nBla.");
    ...
    assert( dialog(2, "Warning!", "This is a test.\nIs this dialog visible?" ) );
    assert(~puts("Ok") );
}
#endif
