// GJK distance algorithm. original code by @vurtun, public domain.
...
API int gjk(gjk_simplex *s, const gjk_support *sup, vec3 *dv);
API gjk_result gjk_analyze(const gjk_simplex *s);
API gjk_result gjk_quad(float a_radius, float b_radius);
