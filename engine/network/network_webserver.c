...
API int webserver(int port, const char *folder);
...
#define READ(fd,buf,len) socket_recv(fd,buf,len)
#define WRITE(fd,buf,len) socket_send(fd,buf,len)
#define CLOSE(fd) socket_close(fd)
#define SLEEP(ss) Sleep((ss)*1000)
#endif

//#include "engine.h" // LOG()
...
bool (weblogger)(int type, const char *s1, const char *s2, int socket_fd) {
    ...
    else if( type == WEB_NOTFOUND) { WRITE(socket_fd, notfound, 224); sprintf(logbuffer, "NOT FOUND: %s:%s", s1, s2); }
    else if( type == WEB_FORBIDDEN) { WRITE(socket_fd, forbidden, 271); sprintf(logbuffer, "WEB_FORBIDDEN: %s:%s", s1, s2); }
...
static
void nweb23(int fd, int hit) {
    int j, buflen;
    long i, ret, len;
    char *fstr;
    static char buffer[WEB_BUFSIZE + 1]; /* static so zero filled */

    ret = READ(fd, buffer, WEB_BUFSIZE); /* read Web request in one go */
    ...
    if( !strcmp(fname, "index.html") ) {
        ...
        while( ptr < end ) {
            WRITE(fd, ptr, (ptr + WEB_BUFSIZE) >= end ? end - ptr : WEB_BUFSIZE);
            ptr += WEB_BUFSIZE;
        }
        goto clean_up;
    }
...
int webserver(int port, const char *path) {
    ...
    if( BIND(listenfd, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) < 0) {
        webpanic("!Cannot bind() to listening socket");
    }
    if( LISTEN(listenfd, 64) < 0) {
        webpanic("!Cannot listen() on listening socket");
    }
#else
    socket_init();
    char url[256]; sprintf(url, "tcp://0.0.0.0:%d", port);
    int listenfd = socket_connect( url, SOCKET_BIND | SOCKET_BLOCK ), socketfd;
    if( listenfd == 0 ) {
        webpanic("!Cannot create listening socket");
    }
#endif
...
#else
        if((socketfd = socket_accept(listenfd)) != 0 ) {
            webpanic("!Cannot accept() listening socket");
#endif
        }
        nweb23(socketfd, hit); /* never returns */
        CLOSE(socketfd);
    }
