...
int main(int argc, char **argv) {
    ...
    /* entities to spawn */
    #ifndef ECS_N
    int ECS_N = argc > 1 ? atoi(argv[1]) : 100000;
    #endif

    /* frames to benchmark */
    #ifndef ECS_F
    int ECS_F = argc > 2 ? atoi(argv[2]) : 1000;
    #endif
    ...
    assert( components(player)[POSITION]->x == (ECS_F * 1));
    assert( components(player)[POSITION]->y == (ECS_F * 2));
}
