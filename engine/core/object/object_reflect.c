...
void* reflect_field( const char *type, void *obj, const char *name );
...
void *reflect_field( const char *base, void *obj, const char *name ) {
    if( base ) for( int i = 0; i < reflect_counter; ++i ) {
        if( registry[i]->internal_type == REFLECT_TYPE_FIELD && strcmp(registry[i]->base, base) ) continue;
        if(!strcmp(registry[i]->name, name) ) return (char*)obj + (long long int)registry[i]->any;
    }
    return 0;
}
