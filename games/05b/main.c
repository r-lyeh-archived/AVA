...
#include "../gl_portable/gl_portable.c"
...
#define STBI_ASSERT   stdlib_assert
#define STBI_MALLOC   stdlib_malloc
#define STBI_REALLOC  stdlib_realloc
#define STBI_FREE     stdlib_free
#define STBI_ABS      stdlib_abs
#include "stb_image.h"
#endif
...
int main() {
  ...
  unsigned char * cubemap_px_file_mem = file_read("cubemap/cubemap_px_right.png");  size_t cubemap_px_size = file_size("cubemap/cubemap_px_right.png");
  unsigned char * cubemap_nx_file_mem = file_read("cubemap/cubemap_nx_left.png");   size_t cubemap_nx_size = file_size("cubemap/cubemap_nx_left.png");
  unsigned char * cubemap_py_file_mem = file_read("cubemap/cubemap_py_top.png");    size_t cubemap_py_size = file_size("cubemap/cubemap_py_top.png");
  unsigned char * cubemap_ny_file_mem = file_read("cubemap/cubemap_ny_bottom.png"); size_t cubemap_ny_size = file_size("cubemap/cubemap_ny_bottom.png");
  unsigned char * cubemap_pz_file_mem = file_read("cubemap/cubemap_pz_front.png");  size_t cubemap_pz_size = file_size("cubemap/cubemap_pz_front.png");
  unsigned char * cubemap_nz_file_mem = file_read("cubemap/cubemap_nz_back.png");   size_t cubemap_nz_size = file_size("cubemap/cubemap_nz_back.png");

  int cubemap_w = 0, cubemap_h = 0, cubemap_bpp = 0;
  void * cubemap_px = stbi_load_from_memory(cubemap_px_file_mem, cubemap_px_size, &cubemap_w, &cubemap_h, &cubemap_bpp, 4/*STBI_rgb_alpha*/);
  void * cubemap_nx = stbi_load_from_memory(cubemap_nx_file_mem, cubemap_nx_size, &cubemap_w, &cubemap_h, &cubemap_bpp, 4/*STBI_rgb_alpha*/);
  void * cubemap_py = stbi_load_from_memory(cubemap_py_file_mem, cubemap_py_size, &cubemap_w, &cubemap_h, &cubemap_bpp, 4/*STBI_rgb_alpha*/);
  void * cubemap_ny = stbi_load_from_memory(cubemap_ny_file_mem, cubemap_ny_size, &cubemap_w, &cubemap_h, &cubemap_bpp, 4/*STBI_rgb_alpha*/);
  void * cubemap_pz = stbi_load_from_memory(cubemap_pz_file_mem, cubemap_pz_size, &cubemap_w, &cubemap_h, &cubemap_bpp, 4/*STBI_rgb_alpha*/);
  void * cubemap_nz = stbi_load_from_memory(cubemap_nz_file_mem, cubemap_nz_size, &cubemap_w, &cubemap_h, &cubemap_bpp, 4/*STBI_rgb_alpha*/);
  ...
  if (cubemap_px != NULL) { stbi_image_free(cubemap_px); cubemap_px = NULL; }
  if (cubemap_nx != NULL) { stbi_image_free(cubemap_nx); cubemap_nx = NULL; }
  if (cubemap_py != NULL) { stbi_image_free(cubemap_py); cubemap_py = NULL; }
  if (cubemap_ny != NULL) { stbi_image_free(cubemap_ny); cubemap_ny = NULL; }
  if (cubemap_pz != NULL) { stbi_image_free(cubemap_pz); cubemap_pz = NULL; }
  if (cubemap_nz != NULL) { stbi_image_free(cubemap_nz); cubemap_nz = NULL; }

  if (cubemap_px_file_mem != NULL) { free(cubemap_px_file_mem); cubemap_px_file_mem = NULL; }
  if (cubemap_nx_file_mem != NULL) { free(cubemap_nx_file_mem); cubemap_nx_file_mem = NULL; }
  if (cubemap_py_file_mem != NULL) { free(cubemap_py_file_mem); cubemap_py_file_mem = NULL; }
  if (cubemap_ny_file_mem != NULL) { free(cubemap_ny_file_mem); cubemap_ny_file_mem = NULL; }
  if (cubemap_pz_file_mem != NULL) { free(cubemap_pz_file_mem); cubemap_pz_file_mem = NULL; }
  if (cubemap_nz_file_mem != NULL) { free(cubemap_nz_file_mem); cubemap_nz_file_mem = NULL; }
  ...
  for (float pos_x = 0, pos_y = 0, pos_z = -2, rot_x = 0, rot_y = 0; glfwWindowShouldClose(window) == 0; glfwPollEvents()) {
    double mouse_x = 0;
    double mouse_y = 0;
