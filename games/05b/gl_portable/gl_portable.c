...
#ifdef _WIN32
#define GLDEF(ret,name,args) ret (APIENTRY *name) args = 0;
#define GLIMP(ret,name,args) name = (void*)glfwGetProcAddress(#name);
GLXMACROS(GLDEF);
void glInitWin32() {
  GLXMACROS(GLIMP);
  if(!glDepthRangef) glDepthRangef = (void*)glfwGetProcAddress("glDepthRange");
  if(!glClearDepthf) glClearDepthf = (void*)glfwGetProcAddress("glClearDepth");
}
#endif
