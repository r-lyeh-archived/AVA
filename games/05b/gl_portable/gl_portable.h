...
struct GL_GRAPHICS_PIPELINE_STATE_INFO {
  ...
  enum GL_CULL_MODE    rasterizationCullMode;
  enum GL_FRONT_FACE   rasterizationFrontFace;
  ...
  enum GL_COMPARE_OP   depthStencilDepthCompareOp;
  GlBool32             depthStencilStencilTestEnable;
  enum GL_STENCIL_OP   depthStencilFrontAndBackFailOp;
  enum GL_STENCIL_OP   depthStencilFrontAndBackPassOp;
  enum GL_STENCIL_OP   depthStencilFrontAndBackDepthFailOp;
  enum GL_COMPARE_OP   depthStencilFrontAndBackCompareOp;
  ...
  enum GL_LOGIC_OP     colorBlendLogicOp;
  float                colorBlendBlendConstants[4];
  GlBool32             colorBlendBlendEnable;
  enum GL_BLEND_FACTOR colorBlendSrcColorAndAlphaBlendFactor;
  enum GL_BLEND_FACTOR colorBlendDstColorAndAlphaBlendFactor;
  enum GL_BLEND_OP     colorBlendColorAndAlphaBlendOp;
...
// --- (ugly) win32 port below

void glInitWin32();

#define GLXMACROS(X) \
\
X(void, glEnable, (unsigned cap));\
X(void, glDisable, (unsigned cap));\
\
X(void, glGenProgramsARB, (int n, unsigned *programs));\
X(void, glBindProgramARB, (enum GL_PROGRAM_TARGET target, unsigned program));\
X(void, glProgramStringARB, (enum GL_PROGRAM_TARGET target, enum GL_PROGRAM_FORMAT format, int len, const void *string));\
X(void, glDeleteProgramsARB, (int n, const unsigned *programs));\
X(void, glProgramEnvParameter4fARB, (enum GL_PROGRAM_TARGET target, unsigned index, float x, float y, float z, float w));\
X(void, glProgramLocalParameter4fARB, (enum GL_PROGRAM_TARGET target, unsigned index, float x, float y, float z, float w));\
\
X(unsigned, glGenLists, (int range));\
X(void, glDeleteLists, (unsigned list, int range));\
X(void, glNewList, (unsigned list, enum GL_LIST_MODE mode));\
X(void, glEndList, (void));\
X(void, glListBase, (unsigned base));\
X(void, glCallList, (unsigned list));\
X(void, glCallLists, (int n, unsigned type, const void *lists));\
\
X(void, glBegin, (enum GL_PRIMITIVE_TOPOLOGY topology));\
X(void, glEnd, (void));\
X(void, glVertex4f, (float x, float y, float z, float w));\
X(void, glNormal3f, (float nx, float ny, float nz));\
X(void, glColor4f, (float red, float green, float blue, float alpha));\
X(void, glMultiTexCoord4f, (unsigned target, float s, float t, float r, float q));\
\
X(void, glActiveTexture, (unsigned texture));\
\
X(void, glGenTextures, (int n, unsigned *textures));\
X(void, glDeleteTextures, (int n, const unsigned *textures));\
X(void, glBindTexture, (enum GL_TEXTURE_TARGET target, unsigned texture));\
X(void, glTexParameteri, (enum GL_TEXTURE_TARGET target, enum GL_TEXTURE_PARAMETER_NAME pname, int param));\
\
X(void, glPixelStorei, (enum GL_PIXEL_STORE_PARAMETER_NAME pname, int param));\
\
X(void, glTexImage2D, (enum GL_TEXTURE_TARGET target, int level, enum GL_TEXTURE_TEXEL_FORMAT internalformat, int width, int height, int must_be_0, enum GL_TEXTURE_TEXEL_FORMAT format, enum GL_TEXTURE_TEXEL_TYPE type, const void *pixels));\
X(void, glTexSubImage2D, (enum GL_TEXTURE_TARGET target, int level, int xoffset, int yoffset, int width, int height, unsigned format, enum GL_TEXTURE_TEXEL_TYPE type, const void *pixels));\
X(void, glCopyTexImage2D, (enum GL_TEXTURE_TARGET target, int level, enum GL_TEXTURE_TEXEL_FORMAT internalformat, int x, int y, int width, int height, int must_be_0));\
X(void, glCopyTexSubImage2D, (enum GL_TEXTURE_TARGET target, int level, int xoffset, int yoffset, int x, int y, int width, int height));\
\
X(void, glReadPixels, (int x, int y, int width, int height, enum GL_TEXTURE_TEXEL_FORMAT format, enum GL_TEXTURE_TEXEL_TYPE type, void *pixels));\
\
X(void, glDepthRangef, (float nearVal, float farVal));\
\
X(void, glClearDepthf, (float depth));\
X(void, glClearStencil, (int s));\
\
X(void, glViewport, (int x, int y, int width, int height));\
X(void, glScissor, (int x, int y, int width, int height));\
\
X(void, glClearColor, (float red, float green, float blue, float alpha));\
X(void, glClear, (enum GL_CLEAR_MASK mask));\
\
X(void, glFlush, (void));\
X(void, glFinish, (void));\
\
X(void, glGetIntegerv, (unsigned pname, int *params));\
X(const unsigned char *, glGetString, (unsigned name));\
X(enum GL_GET_ERROR, glGetError, (void));\
\
/* Pipeline state: */\
\
X(void, glBlendFunc, (unsigned sfactor, unsigned dfactor));\
X(void, glColorMask, (unsigned char red, unsigned char green, unsigned char blue, unsigned char alpha));\
\
X(void, glCullFace, (unsigned mode));\
X(void, glFrontFace, (unsigned mode));\
X(void, glPolygonOffset, (float factor, float units));\
\
X(void, glDepthMask, (unsigned char flag));\
X(void, glDepthFunc, (unsigned func));\
X(void, glStencilMask, (unsigned mask));\
X(void, glStencilOp, (unsigned fail, unsigned zfail, unsigned zpass));\
X(void, glStencilFunc, (unsigned func, int ref, unsigned mask));\
\
X(void, glBlendColor, (float red, float green, float blue, float alpha)); \
X(void, glBlendEquation, (unsigned mode)); \
;

#ifdef _WIN32
#include <winsock2.h> // APIENTRY
#define GLDECL(ret,name,args) extern ret (APIENTRY *name) args;
#define GLFW_INCLUDE_NONE
#include <GLFW/glfw3.h>
#define glfwMakeContextCurrent(window) do { glfwMakeContextCurrent(window); glInitWin32(); } while(0)
#else
#define GLDECL(ret,name,args) ret name args;
#endif
