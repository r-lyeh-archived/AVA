...
static int(*NtDelayExecution)(BOOL Alertable, PLARGE_INTEGER DelayInterval) = (int (__cdecl *)(BOOL,PLARGE_INTEGER))GetProcAddress(GetModuleHandleA("ntdll.dll"), "NtDelayExecution");
static int(*ZwSetTimerResolution)(IN ULONG RequestedResolution, IN BOOLEAN Set, OUT PULONG ActualResolution) = (int (__cdecl *)(ULONG,BOOLEAN,PULONG))GetProcAddress(GetModuleHandleA("ntdll.dll"), "ZwSetTimerResolution");
...
int usleep(unsigned long usec) {
    ...
    closesocket(s);
    return select(0, 0, 0, &dummy, &tv); /* return bool: 0 == select() */
}
...
void editor_draw() {
    ...
    static bool threaded = 0; if( !threaded ) { threaded = 1;
        ...
        std::thread( network_update ).detach();
    }
